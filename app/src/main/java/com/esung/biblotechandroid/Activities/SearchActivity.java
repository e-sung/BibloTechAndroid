...
public class SearchActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    private void fillInRecyclerView(String query) {
        Call<List<BookSearchResults>> call = null;
        switch (SearchCriteria){
            case SEARCH_BY_TITLE:
               call = mNodeJsService.getBooksTitled(query);
                break;
            case SEARCH_BY_AUTHOR:
                call = mNodeJsService.getBooksWrittenBy(query);
                break;
            case SEARCH_BY_PUBLISHER:
                call = mNodeJsService.getBooksPublishedBy(query);
        }
